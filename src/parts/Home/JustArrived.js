...
export default function JustArrived(item, index) {
	...
	console.log('justarive: ', data, status, error);
						...
						{data.data.map((item) => {
							return (
								<div key={item.id} className="px-4 relative card group">
									...
									<Link
										to={`/categories/${item.code_sku}/products/${item.id}`}
